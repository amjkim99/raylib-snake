...
void Game::draw_basic() {
  ...
  DrawRectangleLinesEx(Rectangle{OFFSET - 5, OFFSET - 5, CELL_SIZE * CELL_COUNT + 10, CELL_SIZE * CELL_COUNT + 10},
                       5, GREEN);
...
void Game::draw_title_screen() {
  draw_basic();
  DrawText("Arrow keys to play", SCREEN_WIDTH / 2 - MeasureText("Arrow keys to play", 35) / 2,
           SCREEN_HEIGHT * 0.75 - 10, 35, GREEN);
  DrawText("TAB to change difficulty", SCREEN_WIDTH / 2 - MeasureText("TAB to change difficulty", 30) / 2,
           SCREEN_HEIGHT * 0.75 + 30, 30, GREEN);
...
void Game::draw_lose() {
  ...
  DrawText(TextFormat("%i", score), SCREEN_WIDTH / 2 - MeasureText("SCORE: ", 35) / 2 + 125,
           SCREEN_HEIGHT * 0.5 + 20, 35, GREEN);
...
bool Game::good_direction(KeyboardKey key) const {
  switch (key) {
    default:
      break;
    case KEY_UP:
